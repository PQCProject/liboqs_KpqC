...
#ifdef OQS_ENABLE_KEM_ntru_plus_kem576
        OQS_KEM_alg_ntru_plus_kem576_opt,
#endif
...
OQS_API int OQS_KEM_alg_is_enabled(const char *method_name) {
    if (method_name == NULL) {
        ...
        return 0;
#endif
    // [수정 3] 사용자 브랜치: Opt 버전 활성화 여부
    } else if (0 == strcasecmp(method_name, OQS_KEM_alg_ntru_plus_kem576_opt)) {
#ifdef OQS_ENABLE_KEM_ntru_plus_kem576
        return 1;
#else
        return 0;
#endif
    // [수정 4] Main 브랜치: Neon 버전 활성화 여부
...
OQS_API OQS_KEM *OQS_KEM_new(const char *method_name) {
    ...
    // [수정 5] 사용자 브랜치: Opt 버전 생성
    else if (0 == strcasecmp(method_name, OQS_KEM_alg_ntru_plus_kem576_opt)) {
#ifdef OQS_ENABLE_KEM_ntru_plus_kem576
        return OQS_KEM_ntru_plus_kem576_opt_new();
#else
        return NULL;
#endif
    }
    // [수정 6] Main 브랜치: Neon 버전 생성
    else if (0 == strcasecmp(method_name, OQS_KEM_alg_ntru_plus_kem576_neon)) {
#ifdef OQS_ENABLE_KEM_ntru_plus_kem576_neon
